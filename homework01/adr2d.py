...
def main(arguments):
    ...
    for i in range(nsteps):
        ...
        if i % 250 == 0:
            output_state("adr2d-state-log" + str(i) + ".dat", states[i])

    sys.stdout.write("\rCalculating states... done.\n")

    output_state('initial.dat', make_initial())
...
def output_state(filename, state):
    #print(state)
    #print(filename)
    #print(N)
    state = state.reshape((N, N))
    #print(state)
    l = np.linspace(-N*d/2, N*d/2, N)
    x, y = np.meshgrid(l, l)
    with open(filename, 'w') as f:
        for i in range(N):
            for j in range(N):
                ...
                #print(y[j][i])
                #print(state[j][i])
                f.write(str(x[j][i]) + ' ' + str(y[j][i]) + ' ' + str(state[j][i]) + '\n')
            f.write('\n') 
